...
module file_handling_class
    ...
    function open_unformatted_outfile(file_name) result(unit_no)
        character(len=*), intent(in) :: file_name     !< path to open
        integer                      :: unit_no       !< unit number
        integer istatus

       unit_no= get_next_io_unit()
       open (unit=unit_no, file=trim(file_name), status="unknown",form='unformatted',iostat=istatus)
       if (istatus /= 0) call raise_exception("Could not open file: "//&
                                              trim(adjustl(file_name)),"open_unformatted_outfile",&
                                              180004)
    end function open_unformatted_outfile



    !> Open an unformatted file for reading
    function open_unformatted_infile(file_name) result(unit_no)
        character(len=*), intent(in) :: file_name     !< path to open
        integer                      :: unit_no       !< unit number
        integer istatus

       unit_no= get_next_io_unit()
       open (unit=unit_no, file=trim(file_name), status="old",form='unformatted',iostat=istatus)
       if (istatus /= 0) call raise_exception("Could not open file: "//&
                                              trim(adjustl(file_name)),"open_unformatted_infile",&
                                              180004)
    end function open_unformatted_infile
