...
for f in folders:
    ...
    X_all[A] += X
    ax.plot(A,X,color='grey',zorder=1)
...
ax.plot(np.nan,np.nan,color='grey',zorder=1,label="Individual mass fractions")
...
ax.set_ylim(1e-6,1)
ax.set_xlim(100,245)
plt.savefig("massfractions_nsm_dyn_ejecta_ns10ns10_rosswog.pdf",bbox_inches="tight")
plt.show()
